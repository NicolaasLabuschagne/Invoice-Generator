...
export default function EditInvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [clientsRes, allJobsRes, invoiceRes, profileRes] = await Promise.all([
        fetch('/api/clients'),
        fetch('/api/jobs'),
      ...
      if (invoiceData) {
        ...
        setDiscountAmountValue(invoiceData.discountAmount?.toString() || '0')
        setStatus(invoiceData.status)
        if (invoiceData.jobs) {
          setSelectedJobIds(invoiceData.jobs.map((j: any) => j.id))
        }
      }
      setLoading(false)
    }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center space-x-4">
        ...
        <h1 className="text-3xl font-bold text-slate-900">Edit Invoice {invoiceNumber}</h1>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
            <h2 className="text-sm font-medium text-slate-700 mb-4">Included Jobs</h2>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 sticky top-8">
            ...
            <div className="space-y-4 mb-8">
              ...
              <div className="pt-4 border-t border-slate-100 space-y-2">
                ...
                {(pctDiscountAmount > 0 || fixedDiscountAmount > 0) && (
                  <div className="pt-2 space-y-1">
                    {pctDiscountAmount > 0 && (
                      <div className="flex justify-between text-xs text-red-600 font-medium">
                        <span>{discount}% Off:</span>
                        <span>-{currency}{pctDiscountAmount.toFixed(2)}</span>
                      </div>
                    )}
                    {fixedDiscountAmount > 0 && (
                      <div className="flex justify-between text-xs text-red-600 font-medium">
                        <span>Fixed Discount:</span>
                        <span>-{currency}{fixedDiscountAmount.toFixed(2)}</span>
                      </div>
                    )}
                  </div>
                )}
